...
**Status (2026-09-22):** In progress — Chapters 1-6, 9-11 done for real. Chapters 7 (WAF) and 8 (Defender for Cloud) deliberately deferred pending a cost check-in, since both have real paid tiers.
...
- Full chapter content (9 chapters total: Key Vault + Managed Identity, positive/negative Managed Identity verification, the egress regression + NAT Gateway fix, the exposure fix + Azure Policy, shared responsibility, the RBAC audit, secret rotation, CI security scanning, and a least-privilege synthesis) written in the frontend curriculum browser.
- **Not yet done:** WAF and Defender for Cloud chapters — both have real paid tiers, deliberately deferred pending a cost conversation with the user before building anything.
