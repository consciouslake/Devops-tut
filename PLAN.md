...
**Status (2026-09-21):** In progress — Chapters 1-8 done. Chapters 1-6 built on the real `ci.yml` from Module 2 (see prior note). Chapters 7-8 added a real `docker-build-scan` job (Trivy) and hit genuine findings: backend had a CRITICAL CVE (`python-jose` 3.3.0) and several HIGH (`starlette`, pinned old by `fastapi==0.115.0`) — fixed by loosening the `fastapi` pin so pip resolved patched versions, verified clean on rescan; one unfixable transitive CVE (`pyasn1`, capped by `python-jose` itself) documented in `backend/.trivyignore` with reasoning rather than ignored silently. Frontend (`nginx:1.27-alpine`) had 37 HIGH findings, all Alpine OS packages — partially cleared by bumping to `nginx:1.29-alpine`; remaining HIGH findings handled via a differentiated severity policy (CRITICAL-only gate for this image, documented in `ci.yml` comments) rather than a 37-entry ignore list. Also caught a real gap: an initial local Trivy check only looked at `tail`-truncated output and missed most findings — full output review found the true count. Chapter 9 done: asked directly why pay for ACR (~$5/mo) when GitHub already hosts the code — worked through the real tradeoff (ACR's genuine advantages, Private Endpoint + Managed Identity, aren't exercised by this project's architecture yet) and chose **GitHub Container Registry (ghcr.io) instead of ACR**, zero extra cost, simpler auth (`GITHUB_TOKEN`, no OIDC needed just to push). `ci.yml`'s `docker-build-scan` job now publishes both images to `ghcr.io` on real merges to `main` only (PRs still get full validate — build+scan — but never publish), guarded by `permissions: packages: write`. Revisit point tracked for Module 9 (AKS) if private-network image pulls become a real need. Chapters 10-11 (OIDC-authenticated Azure deploy, rollback/approvals) remaining. Full chapter content for 1-9 written in the frontend curriculum browser. See LEARNING_LOG.md "Module 7" for details.
