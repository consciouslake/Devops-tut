...
**Update, 2026-09-25 (staging tier + pipeline hardening):** two follow-on passes, both real, both against the live pipeline and cluster:
- **Staging tier**: `deploy-staging` now runs before `deploy-production` even becomes available (`needs: deploy-staging`) — deploys the exact scanned commit SHA to a new `azureops-copilot-staging` namespace on the same 3-node cluster ($0 marginal cost, no new VM), smoke-tests it through a real `staging.devopspk.online` endpoint (new DNS record, new Traefik route sharing the existing IngressRoute/cert to avoid Module 12's ACME-race bug), then only production's manual-approval gate opens. Deploys switched from `kubectl rollout restart` against mutable `:latest` to `kubectl set image` against the exact SHA in both stages. Hit the exact class of bug Chapter 11 already documented: `environment: staging`'s OIDC subject (`...:environment:staging`) had no matching Federated Credential — fixed with a third one, alongside the existing `ref:refs/heads/main` and `environment:production` credentials.
- **Automated rollback**: a failed post-deploy health check in `deploy-production` now triggers `kubectl rollout undo` automatically instead of just failing the job — production is left on the last-good image, not the broken one.
- **A real CI bug, found and fixed**: adding SARIF output (`format: sarif`) to the same Trivy step as the existing severity gate silently dropped `severity`/`ignore-unfixed`/`exit-code`/`trivyignores` entirely — `trivy-action` scans "with all severities" whenever SARIF output is requested, confirmed from the real failed run's logged command (`trivy image <ref>`, no flags at all). Fixed by splitting gating and SARIF reporting into two separate `trivy-action` invocations. Verified the image itself was never the problem by rebuilding with `--no-cache` and re-scanning locally with the raw `trivy` binary first.
- **Optimization pass**: real lint jobs added (`ruff` for backend, ESLint for frontend — previously only `npm run build` ran, catching nothing but compile errors; found and fixed one unused import and two unnecessary regex escapes), `docker-build-scan` split into a backend/frontend build matrix (parallel instead of sequential), Trivy's own ~117MB vulnerability DB cached with a daily-rotating key (was re-downloading on all 4 scan steps every run), an SPDX SBOM generated and uploaded per image, and docs-only commits (`*.md`) no longer trigger the pipeline.

Three new chapters added to Module 7 in the frontend curriculum browser (staging/promotion gates, automated rollback, the Trivy SARIF bug), and the existing "Artifacts, caching, matrices" chapter's honest "none of this is built yet" claim corrected now that all three are real.
