...
**Status (2026-09-21):** In progress — Chapters 1-5 done. Chapters 1-4 (VNet/subnet, NSG, public/private connectivity, route tables) built from scratch into `azureops-vnet` (10.10.0.0/16, app-subnet + gateway-subnet). Chapter 5 (Load Balancer) fully built and verified: `azureops-lb` (Standard SKU) load-balancing two real backend VMs (`app-vm1`/`app-vm2`), with real failover tested (stopped `pyapp` on one VM, confirmed 8/8 requests rerouted to the other within ~20s, confirmed automatic rejoin on restart). Two genuine bugs hit and fixed along the way: (1) Git Bash mangled the health probe's `--path /health` into a Windows path, silently marking both backends unhealthy; (2) `az vm create` auto-created NIC-level NSGs on both backend VMs stacking on top of the intended subnet-level `app-subnet-nsg`, blocking all traffic until found via `az network nic list-effective-nsg` and removed. All commands run by the user directly per their request. Chapters 6-10 (Private Link, Application Gateway+WAF, Azure DNS, Front Door, network lab) remaining — paced across multiple days, resources kept running rather than torn down same-day per user preference. Full chapter content for 1-5 written in the frontend curriculum browser. See LEARNING_LOG.md "Module 6 — Azure Networking, Chapter 5" for details.
