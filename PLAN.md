...
**Status (2026-09-22):** In progress — Chapter 5 (TLS/custom domains) done for real via a different real path than originally planned; Chapters 2-4 partially informed by a genuine, hard finding.
...
- **Currently blocked on external DNS propagation, not anything Azure-side:** `devopspk.online`'s registrar NS records still show GoDaddy's (`ns59/ns60.domaincontrol.com`) rather than Azure's, confirmed via two independent resolvers (local + Google's `8.8.8.8`). The live site currently serves Traefik's default self-signed cert for the domain (verified via `openssl s_client`) since Let's Encrypt's HTTP-01 challenge needs the domain to resolve publicly first — this will resolve itself automatically once the registrar delegation propagates (Traefik retries ACME issuance on its own), no further action needed on the Azure/cluster side.
