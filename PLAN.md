...
**Status (2026-09-21):** In progress — Chapters 1-7 done. Chapters 1-4 (VNet/subnet, NSG, public/private connectivity, route tables) built from scratch into `azureops-vnet` (10.10.0.0/16, app-subnet + gateway-subnet). Chapter 5 (Load Balancer) fully built and verified: `azureops-lb` (Standard SKU) load-balancing two real backend VMs (`app-vm1`/`app-vm2`), with real failover tested. Chapter 6 (Private Link) built on Module 5's real storage account, public access disabled and difference verified from both sides. Chapter 7 (Application Gateway/WAF concepts) deliberately built as a **software WAF instead of Azure Application Gateway**, a cost-conscious redirect requested by the user mid-session: `app-vm1`/`app-vm2` each run `owasp/modsecurity-crs:nginx` in Docker, proxying to the local app; `azureops-lb`'s probe and rule re-pointed from port 8000 to 8080 so all traffic passes through WAF inspection. Verified end-to-end through the real public path: normal request succeeded, a SQL-injection-style payload was blocked with HTTP 403 — same protection as Azure's managed WAF, zero extra Azure cost. User explicitly chose to keep both the managed Load Balancer (Chapter 5) and this software WAF running for direct comparison rather than tearing either down. Two genuine bugs hit and fixed in Chapter 5, one path-mangling bug recurred in Chapter 6, one container-privilege port bug hit and fixed in Chapter 7 (unprivileged container can't bind port 80, used 8080 instead). All commands run by the user directly per their request. Chapter 8 (Azure DNS) done: a real public DNS zone (`azureops-lab.test`, deliberately a throwaway IANA-reserved test TLD, not `devopspk.online`) with A/CNAME/TXT records, verified resolving by querying Azure's nameserver directly (bypassing registrar delegation entirely) — `app.azureops-lab.test` resolved to `azureops-lb`'s real IP. `devopspk.online` deliberately untouched, staying deferred to Module 13. Chapter 9 (Front Door) done as concept-and-comparison, no resource built: evaluated Front Door against Cloudflare free tier, Traffic Manager, and "skip it" — chose not to build Front Door since this project is single-region (Front Door's core value, multi-region edge failover, doesn't apply yet) and chose not to build Cloudflare either since it requires the real `devopspk.online` domain, which stays reserved for Module 13. Real cost figures for all options written into the chapter content, not just discussed in chat. Chapter 10 (network architecture lab, a synthesis/review exercise) remaining. Full chapter content for 1-9 written in the frontend curriculum browser. See LEARNING_LOG.md "Module 6 — Azure Networking, Chapter 9" for details.
