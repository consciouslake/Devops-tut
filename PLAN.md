...
**Status (2026-09-22):** In progress — Chapters 3-4 (Managed identities, Key Vault) done for real, plus real work on network security and tagging governance (chapters 5 and 9). Chapters 1-2, 6-8, 10-11 not yet started.
...
- Full chapter content (4 chapters: Key Vault + Managed Identity, positive/negative Managed Identity verification, the egress regression + NAT Gateway fix, the exposure fix + Azure Policy) written in the frontend curriculum browser.
- **Not yet done:** Shared responsibility overview, Entra ID/RBAC review, secret rotation chapter, WAF, Defender for Cloud, CI security scanning writeup, least privilege/threat-aware architecture chapter.
